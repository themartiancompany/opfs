...
export async function testAsync() {
  ...
  const
    downloadRes =
      await downloadTask.response;
  if ( downloadRes.isOk() ) {
    console.assert(
      downloadRes.unwrap() instanceof Response);
    const
      postData =
        (await fs.readTextFile(
                 '/todo.json')).unwrap();
    ...
    console.assert(
      ...
      console.assert(
        (await fs.uploadFile(
                 '/todo.json',
                 mockAll).response).unwrap() instanceof Response);
  }
  else {
    console.assert(
      downloadRes.unwrapErr() instanceof Error);
  }
  {
      ...
      x => {
        console.assert(
          fs.isTempPath(
            x.tempFilePath));
        console.assert(
          x.rawResponse instanceof Response);
    });
    if (downloadRes.isOk()) {
      await fs.remove(
              downloadRes.unwrap().tempFilePath);
    }
  }
  ...
  console.assert(
    ( await fs.zip(
              '/happy')).unwrap().byteLength === ( await fs.readFile(
  ...
  console.assert(
    ( await fs.zipFromUrl(
	      mockZipUrl) ).unwrap().byteLength === ( await fs.readFile(
                                                              '/test-zip.zip') ).unwrap().byteLength);
  // Temp
  console.log(
    `Temporary text file: '${fs.generateTempPath(
                               { basename:
                                   'opfs',
                                 extname:
                                   '.txt' })}'.`);
  console.log(
    `Temporary directory: '${fs.generateTempPath(
                              { isDirectory:
                                  true, })}'.`);
  ( await fs.mkTemp() ).inspect(
      path => {
       console.assert(
         path.startsWith(
           '/tmp/tmp-'));
  ...
  for await ( const { path,
                      handle } of ( await fs.readDir(
                                            fs.ROOT_DIR,
                                            { recursive:
                                                true }) ).unwrap() ) {
    const
      handleLike =
        await fs.toFileSystemHandleLike(
                handle);
    if ( fs.isFileHandleLike(
      ...
      for ( _line of _msg ) {
       ...
       console.log(
         `${ path } is a '${ handleLike.kind }', name = '${ handleLike.name }'.`);
    }
  }
  // Comment the next two lines to
